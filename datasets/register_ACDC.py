...
def load_ACDC_semantic(image_dir, gt_dir):
    ...
    for image_file, label_file in _get_ACDC_files(image_dir, gt_dir):
...
def register_all_ACDC(root):
    ...
    for domain in domains:
        for split in splits:
            dataset_name = f"ACDC_{domain}_fine_sem_seg_{split}"
            image_dir = f"acdc/rgb_anon/{domain}/{split}/"
            gt_dir = f"acdc/gt/{domain}/{split}/"
            ...
            DatasetCatalog.register(
                dataset_name, lambda x=image_dir, y=gt_dir: load_ACDC_semantic(x, y)
            )
...
register_all_ACDC(dataset_path)
