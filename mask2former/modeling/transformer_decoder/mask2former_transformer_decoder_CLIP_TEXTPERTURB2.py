...
class MultiScaleMaskedTransformerDecoder_CLIP_TEXTPERTURB2(nn.Module):
    _version = 2
    ...
    def generate_query_embed_weights(self):
        ...
        for i in range(len(self.cityscapes_classes)):
            ...
            class_string = f"{self.start_words[random_startword_idx[i]]} {self.image_descriptions[random_image_description_idx[i]]} {self.image_types[random_image_type_idx[i]]} {self.class_descriptions[random_class_description_idx[i]]} {self.cityscapes_classes[i][random_class_idx]} {self.suffixes[random_suffix_idx[i]]}"
